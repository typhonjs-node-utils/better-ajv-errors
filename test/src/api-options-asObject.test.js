 ...
 */
describe('betterErrors various options (asObject)', () =>
{
   it(`errors 'entry' no options`, () =>
   {
      expect(BetterErrors.asObject(s_ERRORS)).to.be.deep.equal(s_RESULTS_NO_OPTIONS);
   });
   ...
   if (process.env.CI !== 'true')
   {
      ...
      {
         expect(BetterErrors.asObject(s_ERRORS, { file: s_FILE })).to.be.deep.equal(
   ...
   {
      expect(BetterErrors.asObject(s_ERRORS, { file: s_FILE, highlightCode: false })).to.be.deep.equal(
       s_RESULTS_FILE_WITH_HIGHLIGHTCODE_FALSE);
   });

   it(`errors 'entry' just wrapLength: 20`, () =>
   {
      expect(BetterErrors.asObject(s_ERRORS, { wrapLength: 20 })).to.be.deep.equal(s_RESULTS_WITH_WRAPLENGTH_20);
   });

   it(`errors 'entry' toString() no code frame equals wrapLength: 20`, () =>
   {
      const betterErrors = BetterErrors.asObject(s_ERRORS, { wrapLength: 20 });

      expect(BetterErrors.toString(betterErrors)).to.be.deep.equal(s_RESULTS_WITH_WRAPLENGTH_20["/author"][0].message);
   });
});
